...
import { remutakaBarriers, bounceOffBarriers } from '../src/remutakaBarriers.js';
...
const from=summit.point(track.half,0),to=summit.point(track.half+metres(15),0);
assert.ok(crossesSummitFinish(track,from,to));
assert.ok(crossesSummitFinish(track,to,from));
assert.ok(!crossesSummitFinish(track,summit.point(0,-2000),summit.point(0,2000)),'Through road is not the finish');
...
console.log(`Remutaka PASS: ${profile.length} side profiles, ${rails.length} solid rails, fast rebounds, open entrance, paved support, summit finish and descent`);
